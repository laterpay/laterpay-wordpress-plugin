...
class LaterPay_Admin_Controller extends LaterPay_Abstract_Controller
{
    ...
    public function load_assets() {
        ...
        if ( LaterPay_Browser_Helper::is_ie() && LaterPay_Browser_Helper::get_browser_major_version() <= 9 ) {
            wp_register_script(
                'html5-shim-ie',
    ...
    public function run( $tab = '' ) {
        ...
        switch ( $tab ) {
            ...
            case 'get_started':
                $LaterPay_Get_Started_Controller = new LaterPay_Get_Started_Controller();
            ...
            // render pricing tab
            case 'pricing':
                $LaterPay_Pricing_Controller = new LaterPay_Pricing_Controller();
                $LaterPay_Pricing_Controller->render_page();
                break;

            // render appearance tab
            case 'appearance':
                $LaterPay_Appearance_Controller = new LaterPay_Appearance_Controller();
                $LaterPay_Appearance_Controller->render_page();
                break;

            // render account tab
            case 'account':
                $LaterPay_Account_Controller = new LaterPay_Account_Controller();
    ...
    public function modify_footer() {
        ...
        echo $this->get_text_view( 'partials/admin-footer' );
    }
    ...
    public static function process_ajax_requests() {
        if ( isset( $_POST['form'] ) ) {
            ...
            if ( ! LaterPay_User_Helper::can( 'laterpay_read_post_statistics', null, false ) ) {
