...
class LaterPay_Post_Content_Controller extends LaterPay_Abstract_Controller
{
    ...
    public function init_teaser_content( $post ) {
        ...
        if ( is_array( $meta_value ) && count( $meta_value ) == 0 ) {
            $new_meta_value = LaterPay_String_Helper::truncate(
                $post->post_content,
    ...
    public function view( $content ) {
        ...
        if ( $price > 0 ) {
            ...
            if ( is_single() ) {
                ...
                if ( LaterPay_User_Helper::can( 'laterpay_read_post_statistics', $post_id ) ) {
                    $access = true;
                    $this->get_post_statistics();
                } else if ( LaterPay_User_Helper::user_has_full_access() ) {
                    $access = true;
                }

                // encrypt content for premium content
                $content = LaterPay_File_Helper::get_encrypted_content( $post_id, $content, $access );
                ...
                $this->assign( 'can_show_statistic',         LaterPay_User_Helper::can( 'laterpay_read_post_statistics', $post_id ) && (! LaterPay_Request_Helper::is_ajax() || $laterpay_show_statistics) && LATERPAY_ACCESS_LOGGING_ENABLED && $is_premium_content );
                $this->assign( 'post_content_cached',        LaterPay_Cache_Helper::site_uses_page_caching() );
                $this->assign( 'preview_post_as_visitor',    LaterPay_User_Helper::preview_post_as_visitor($post_id) );
                $this->assign( 'hide_statistics_pane',       LaterPay_User_Helper::statistics_pane_is_hidden() );

                $html = $this->get_text_view( 'single' );
            } else {
                $this->assign( 'teaser_content', $teaser_content );
    ...
    public function modify_footer() {
        // if Ajax request
        if ( (LaterPay_Request_Helper::is_ajax() && isset( $_GET['id'] )) || isset( $_GET['id'] ) ) {
            $postid = $_GET['id'];
        } else {
        ...
        if ( ! empty( $postid ) ) {
            ...
            if ( $price > 0 ) {
                ...
                echo $this->get_text_view( 'partials/identify-iframe' );
            }
        }
    }
    ...
    protected function get_post_statistics() {
        ...
        $LaterPay_Payments_History_Model   = new LaterPay_Payments_History_Model();
        $LaterPay_Post_Views_Model = new LaterPay_Post_Views_Model();

        // get total revenue and total sales
        $total = array();
        $history_total = (array) $LaterPay_Payments_History_Model->get_total_history_by_post_id( $post_id );
        ...
        $history_last30DaysRevenue = (array) $LaterPay_Payments_History_Model->get_last_30_days_history_by_post_id( $post_id );
        ...
        $todayRevenue = array();
        $history_todayRevenue = (array) $LaterPay_Payments_History_Model->get_todays_history_by_post_id( $post_id );
        ...
        $history_last30DaysVisitors = (array) $LaterPay_Post_Views_Model->get_last_30_days_history( $post_id );
        ...
        $todayVisitors = (array) $LaterPay_Post_Views_Model->get_todays_history( $post_id );
    ...
    public static function get_post_price( $post_id ) {
        ...
        switch ( $post_price_type ) {
            ...
            case 'individual price, dynamic':
                $price = self::get_dynamic_price( $GLOBALS['post'] );
                break;
    ...
    public static function buy_post() {
        if ( 'index.php' == $GLOBALS['pagenow'] ) {
            if ( isset( $_GET['buy'] ) && $_GET['buy'] ) {
                ...
                $data['hash']           = $_GET['hash'];
                $hash = $_GET['hash'];
                $url = LaterPay_Request_Helper::get_current_url();
    ...
    public static function token_hook() {
        ...
        LaterPay_Logger::debug(
            ...
            array(
                ...
                ! self::is_cron_page(),
                ! $is_feed,
                LaterPay_Browser_Helper::browser_supports_cookies(),
                ! LaterPay_Browser_Helper::is_crawler()
            )
        );

        if ( ! is_admin() && ! self::is_login_page() && ! self::is_cron_page() && ! $is_feed && LaterPay_Browser_Helper::browser_supports_cookies() && ! LaterPay_Browser_Helper::is_crawler() ) {

            LaterPay_Logger::debug( 'LaterPay_Post_Content_Controller::token_hook', array( $_SERVER['REQUEST_URI'] ) );
    ...
    public static function get_laterpay_link( $post_id ) {
        ...
        $LaterPay_Currency_Model = new LaterPay_Currency_Model();
        $LaterPay_Client = new LaterPay_Client();
