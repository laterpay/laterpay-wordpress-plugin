...
class LaterPayPostPricingController extends LaterPayAbstractController
{
    ...
    public function render_post_pricing_form( $object ) {
        ...
        if ( ! empty( $categories_of_post ) ) {
            ...
            $category_price_data    = $LaterPayModelCategory->get_category_price_data_by_category_ids( $categories_of_post );
            ...
            if ( $post_default_category > 0 ) {
                $category_default_price = (float) $LaterPayModelCategory->get_price_by_category_id( $post_default_category );
            }
        }
        ...
        switch ( $post_price_type ) {
            ...
            case 'individual price, dynamic':
                // current price
                $price = LaterPayPostContentController::get_dynamic_price( $GLOBALS['post'] );
                break;
    ...
    protected static function _get_category_prices( $category_ids ) {
        $LaterPayModelCategory = new LaterPayModelCategory();
        $categories_price_data = $LaterPayModelCategory->get_category_price_data_by_category_ids( $category_ids );
