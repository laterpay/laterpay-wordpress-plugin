...
class LaterPayPostContentController extends LaterPayAbstractController
{
    ...
    public function view( $content ) {
        ...
        if ( $price > 0 ) {
            ...
            if ( is_single() ) {
                ...
                $content = LaterPayFileHelper::get_encrypted_content( $post_id, $content, $access );
                ...
                $this->assign( 'can_show_statistic',         LaterPayUserHelper::can( 'laterpay_read_post_statistics', $post_id ) && (! LaterPayRequestHelper::is_ajax() || $laterpay_show_statistics) && LATERPAY_ACCESS_LOGGING_ENABLED && $is_premium_content );
                $this->assign( 'post_content_cached',        LaterPayCacheHelper::site_uses_page_caching() );
                $this->assign( 'preview_post_as_visitor',    LaterPayUserHelper::preview_post_as_visitor($post_id) );
                $this->assign( 'hide_statistics_pane',       LaterPayUserHelper::statistics_pane_is_hidden() );
    ...
    public function modify_footer() {
        // if Ajax request
        if ( (LaterPayRequestHelper::is_ajax() && isset( $_GET['id'] )) || isset( $_GET['id'] ) ) {
            $postid = $_GET['id'];
        } else {
        ...
        if ( ! empty( $postid ) ) {
            ...
            if ( $price > 0 ) {
                ...
                $identify_link = $LaterPayClient->get_identify_url();
    ...
    protected function get_post_statistics() {
        ...
        $total = array();
        $history_total = (array) $LaterPayModelHistory->get_total_history_by_post_id( $post_id );
        ...
        $history_last30DaysRevenue = (array) $LaterPayModelHistory->get_last_30_days_history_by_post_id( $post_id );
        ...
        $todayRevenue = array();
        $history_todayRevenue = (array) $LaterPayModelHistory->get_todays_history_by_post_id( $post_id );
        ...
        $history_last30DaysVisitors = (array) $LaterPayModelPostViews->get_last_30_days_history( $post_id );
        ...
        $todayVisitors = (array) $LaterPayModelPostViews->get_todays_history( $post_id );
    ...
    public static function get_post_price( $post_id ) {
        ...
        switch ( $post_price_type ) {
            ...
            case 'category default price':
                ...
                $price                  = $LaterPayModelCategory->get_price_by_category_id( (int) $category_id );
                break;
        ...
        if ( 'index.php' == $GLOBALS['pagenow'] ) {
            if ( isset( $_GET['buy'] ) && $_GET['buy'] ) {
                ...
                if ( md5( md5( $url ) . LATERPAY_SALT ) == $hash ) {
                    $LaterPayModelHistory = new LaterPayModelHistory();
                    $LaterPayModelHistory->set_payment_history( $data );
                }
    ...
    public static function token_hook() {
        ...
        LaterPayLogger::debug(
            'LaterPayPostContentController::token_hook',
            array(
                ! is_admin(),
                ! self::is_login_page(),
                ! self::is_cron_page(),
        ...
        if ( ! is_admin() && ! self::is_login_page() && ! self::is_cron_page() && ! $is_feed && LaterPayBrowserHelper::browser_supports_cookies() && ! LaterPayBrowserHelper::is_crawler() ) {
            ...
            LaterPayLogger::debug( 'LaterPayPostContentController::token_hook', array( $_SERVER['REQUEST_URI'] ) );

            $LaterPayClient = new LaterPayClient();
            if ( isset($_GET['lptoken']) ) {
                $LaterPayClient->set_token( $_GET['lptoken'], true );
            }
            ...
            if ( ! empty($postid) ) {
                $price = self::get_post_price($postid);
                if ( $price > 0 ) {
                    $result = $LaterPayClient->get_access( $postid );
                    $access = false;
    ...
    public static function get_laterpay_link( $post_id ) {
        ...
        $data = array(
            'post_id'     => $post_id,
            'id_currency' => $LaterPayModelCurrency->get_currency_id_by_iso4217_code( $currency ),
            'price'       => $price,
            'date'        => time(),
            'buy'         => 'true',
        ...
        return $LaterPayClient->get_add_url( $params );
    }
    ...
    public function modify_post_title( $title ) {
        if ( in_the_loop() ) {
            ...
            $link                    = self::get_laterpay_link( $post_id );
            $preview_post_as_visitor = LaterPayUserHelper::preview_post_as_visitor( $post );
            $post_content_cached     = LaterPayCacheHelper::site_uses_page_caching();

            if ( $is_premium_content && is_single() && ! is_page() ) {
                if ( $post_content_cached && ! LaterPayRequestHelper::is_ajax() ) {
                    $this->assign( 'post_id', $post_id );
                    ...
                    if ( (! $access || $preview_post_as_visitor) ) {
                        ...
                        $purchase_button   .= sprintf(
                                                    ...
                                                    LaterPayViewHelper::format_number( $price, 2 ),
                                                    $currency
                                                );
                        $purchase_button   .= '</a>';
