...
class LaterPay
{
    ...
    /**
     * @var LaterPay_Post_Pricing_Controller
     */
    private $_LaterPay_Post_Pricing_Controller;
    /**
     * @var LaterPay_Admin_Controller
     */
    private $_LaterPay_Admin_Controller;
    /**
     * @var LaterPay_Post_Content_Controller
     */
    private $_LaterPay_Post_Content_Controller;
    /**
     * @var LaterPay_GitHub_Plugin_Updater
     */
    private $_GitHub_Plugin_Updater;
    ...
    protected function get_laterpay_post_pricing_controller() {
        if ( empty( $this->_LaterPay_Post_Pricing_Controller ) ) {
            $this->_LaterPay_Post_Pricing_Controller = new LaterPay_Post_Pricing_Controller();
        }

        return $this->_LaterPay_Post_Pricing_Controller;
    }

    protected function get_laterpay_admin_controller() {
        if ( empty( $this->_LaterPay_Admin_Controller ) ) {
            $this->_LaterPay_Admin_Controller = new LaterPay_Admin_Controller();
        }

        return $this->_LaterPay_Admin_Controller;
    }

    protected function get_laterpay_post_content_controller() {
        if ( empty( $this->_LaterPay_Post_Content_Controller ) ) {
            $this->_LaterPay_Post_Content_Controller = new LaterPay_Post_Content_Controller();
        }

        return $this->_LaterPay_Post_Content_Controller;
    }

    protected function get_github_plugin_updater() {
        if ( empty( $this->_GitHub_Plugin_Updater ) ) {
            $this->_GitHub_Plugin_Updater = new LaterPay_GitHub_Plugin_Updater();
            $this->_GitHub_Plugin_Updater->init(
                $this->_pluginFile,
        ...
        return $this->_GitHub_Plugin_Updater;
    }
    ...
    public function activate() {
        ...
        // clear opcode cache
        LaterPay_Cache_Helper::reset_opcode_cache();
    ...
    public function add_to_admin_panel() {
        ...
        $page_number = 0;
        foreach ( LaterPay_View_Helper::$admin_menu as $name => $page ) {
    ...
    protected function setup_admin_routes() {
        if ( class_exists( 'LaterPay_Get_Started_Controller' ) ) {
            add_action( 'wp_ajax_getstarted',    'LaterPay_Get_Started_Controller::process_ajax_requests' );
        }
        if ( class_exists( 'LaterPay_Pricing_Controller' ) ) {
            add_action( 'wp_ajax_pricing',       'LaterPay_Pricing_Controller::process_ajax_requests' );
        }
        if ( class_exists( 'LaterPay_Appearance_Controller' ) ) {
            add_action( 'wp_ajax_appearance',    'LaterPay_Appearance_Controller::process_ajax_requests' );
        }
        if ( class_exists( 'LaterPay_Account_Controller' ) ) {
            add_action( 'wp_ajax_account',       'LaterPay_Account_Controller::process_ajax_requests' );
        }
        if ( class_exists( 'LaterPay_Admin_Controller' ) ) {
            add_action( 'wp_ajax_admin',         'LaterPay_Admin_Controller::process_ajax_requests' );
        }
        if ( class_exists( 'LaterPay_Post_Pricing_Controller' ) ) {
            add_action( 'wp_ajax_post_pricing',  'LaterPay_Post_Pricing_Controller::process_ajax_requests' );
        }
    }
    ...
    public function add_data_to_posts_table( $column_name, $post_id ) {
        if ( $column_name == 'post_price' ) {
            $price      = number_format( (float) LaterPay_Post_Content_Controller::get_post_price( $post_id ), 2 );
            $currency   = get_option( 'laterpay_currency' );
    ...
    public function add_unique_visitors_tracking() {
        ...
        $url    = LaterPay_Statistics_Helper::get_full_url( $_SERVER );
        $postid = url_to_postid($url);
        LaterPay_Statistics_Helper::track($postid);
    }
    ...
    protected function setup_purchasing() {
        add_action( 'init', 'LaterPay_Post_Content_Controller::token_hook' );
        add_action( 'init', 'LaterPay_Post_Content_Controller::buy_post' );
    }
    ...
    public function add_checking_for_updates() {
        ...
        if ( get_option('laterpay_version') != $laterpay_version ) {
            $this->activate();
            $_capabilities = new LaterPay_Capabilities();
