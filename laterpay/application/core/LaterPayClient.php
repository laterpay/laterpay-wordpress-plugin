...
class LaterPayClient {
    ...
    public function getTokenRedirectUrl( $return_to ) {
        ...
        $params = $this->signAndEncode( array( 'redir' => $return_to, 'cp' => $this->cp_key ), $url, Request::GET );
        $url    .= '?' . $params;
    ...
    public function getIframeApiUrl( $next_url, $css_url = null, $forcelang = null, $show_greeting = false, $show_long_greeting = false, $show_login = false, $show_signup = false, $show_long_signup = false, $use_jsevents = false ) {
        ...
        if ( $show_long_greeting ) {
            if ( isset( $data['show'] ) ) {$data['show'] = $data['show'] ;} else { $data['show'] = '' . 'gg'; }
        } elseif ( $show_greeting ) {
            if ( isset( $data['show'] ) ) {$data['show'] = $data['show'] ;} else { $data['show'] = '' . 'g'; }
        }
        if ( $show_login ) {
            if ( isset( $data['show'] ) ) {$data['show'] = $data['show'] ;} else { $data['show'] = '' . 'l'; }
        }
        if ( $show_long_signup ) {
            if ( isset( $data['show'] ) ) {$data['show'] = $data['show'] ;} else { $data['show'] = '' . 'ss'; }
        } elseif ( $show_signup ) {
            if ( isset( $data['show'] ) ) {$data['show'] = $data['show'] ;} else { $data['show'] = '' . 's'; }
        }
    ...
    public function getLoginDialogUrl( $next_url, $use_jsevents = false ) {
        if ( $use_jsevents ) { $aux = '"&jsevents=1'; } else { $aux = ''; }
    ...
    public function getMeteredAccess( $article_ids, $threshold = 5, $product_key = null ) {
        ...
        if ( isset( $data['subs'] ) ) {$subs = $data['subs'] ;} else { $subs = array(); }
     ...
     * Send request to $url
     *
     * @param string  $url    url to sends a request
     * @param array   $params
     * @param string  $method
    ...
    protected function makeRequest( $url, $params = array(), $method = Request::GET ) {
        ...
        try {
            if ( $method == Request::POST ) {
                $requestsResponse = wp_remote_retrieve_body( wp_remote_post( $url, array(
                            'headers'   => $headers,
                            'body'      => $params,
                            'timeout'   => 30,
                        ) ) );
                ...
                $url .= '?' . $params;
                $requestsResponse = $requestsResponse = wp_remote_retrieve_body( wp_remote_get( $url, array(
                            'headers' => $headers,
                            'timeout' => 30,
                        ) ) );
            }
