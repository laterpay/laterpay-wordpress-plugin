...
class LaterPay_Client
{
    ...
    public function __construct( $_args = array() ) {
        ...
        LaterPay_Logger::debug('LaterPay_Client::constructor', array(
    ...
    public function get_identify_url( $identify_callback = null ) {
        ...
        $params = $this->sign_and_encode( $data, $url, LaterPay_Request::GET );
        $url .= '?' . $params;

        LaterPay_Logger::debug('LaterPay_Client::get_identify_url', array(
                            'url'       => $url,
                            'api_key'   => $this->api_key,
                            'cp_key'    => $this->cp_key,
                            'lptoken'   => $this->lptoken,
    ...
    */
    protected function get_web_url( $data, $page_type, $product_key = null, $dialog = true, $use_jsevents = false, $skip_add_to_invoice = false, $transaction_reference = null ) {
        ...
        $params = $this->sign_and_encode( $data, $base_url, LaterPay_Request::GET );
        $url    = $base_url . '?' . $params;

        LaterPay_Logger::debug('LaterPay_Client::get_web_url', array(
    ...
    public function add_metered_access( $article_id, $threshold = 5, $product_key = null ) {
        ...
        $data = $this->make_request( $this->_get_add_url(), $params, LaterPay_Request::POST );
     ...
     */
    public function sign_and_encode( $params, $url, $method = LaterPay_Request::GET ) {
        LaterPay_Logger::debug('LaterPay_Client::sign_and_encode', array($params, $url, $method));
    ...
    public function get_access( $article_ids, $product_key = null ) {
        LaterPay_Logger::debug('LaterPay_Client::get_access', array('checking access', $article_ids));
        ...
        if ( ! in_array( $data['status'], $allowed_statuses ) ) {
            LaterPay_Logger::error( 'get_access::invalid status', array( 'result' => $data ) );
        }

        LaterPay_Logger::debug( 'LaterPay_Client::get_access', array(
                            'params'    => $params,
                            'result'    => $data,
                            'api_key'   => $this->api_key,
                            'cp_key'    => $this->cp_key,
    ...
    public function acquire_token() {
        $link = $this->_get_token_redirect_url( LaterPay_Request_Helper::get_current_url() );

        LaterPay_Logger::debug( 'LaterPay_Client::acquire_token', array(
                            'link'      => $link,
                            'api_key'   => $this->api_key,
                            'cp_key'    => $this->cp_key,
                            'lptoken'   => $this->lptoken,
    ...
    public function set_token( $token, $redirect = false ) {
        LaterPay_Logger::debug( 'LaterPay_Client::set_token', array(
                            'token'     => $token,
                            'redirect'  => $redirect,
                            'api_key'   => $this->api_key,
                            'cp_key'    => $this->cp_key,
        ...
        if ( $redirect ) {
            header( 'Location: ' . LaterPay_Request_Helper::get_current_url() );
            die;
        }
    }

    public function delete_token() {
        LaterPay_Logger::debug( 'LaterPay_Client::delete_token', array(
                            'api_key'   => $this->api_key,
                            'cp_key'    => $this->cp_key,
                            'lptoken'   => $this->lptoken,
                        )
     ...
     */
    protected function make_request( $url, $params = array(), $method = LaterPay_Request::GET ) {
        LaterPay_Logger::debug( 'LaterPay_Client::make_request', array(
                            'url'       => $url,
                            'params'    => $params,
                            'post'      => $post,
                            'api_key'   => $this->api_key,
        ...
        try {
            if ( $method == LaterPay_Request::POST ) {
            ...
            LaterPay_Logger::debug( 'LaterPay_Client::make_request', array( $requestsResponse ) );
            ...
            LaterPay_Logger::error( 'LaterPay_Client::make_request', array(
        ...
        LaterPay_Logger::debug( 'LaterPay_Client::make_request', array( 'response' => $response ) );
