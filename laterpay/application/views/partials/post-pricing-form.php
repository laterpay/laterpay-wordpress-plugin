...
<div id="laterpay-post-price" class="clearfix">
    <p>
        <input type="text"
                ...
                name="post-price"
                value="<?php echo LaterPay_View_Helper::format_number($laterpay_price, 2); ?>"
...
<div id="laterpay-price-type"<?php if ( $laterpay_post_price_type == 'individual price, dynamic' || $laterpay_post_price_type == 'category default price' ) { echo ' class="expanded"'; } ?>>
    <ul class="lp-toggle clearfix">
        ...
        <li class=" <?php if ( $laterpay_post_price_type == 'global default price' || ($laterpay_post_price_type == '' && $laterpay_global_default_price > 0) ) { echo 'selected'; } ?>
            ...
            <a href="#" id="use-global-default-price" class="use-global-default-price" data-price="<?php echo LaterPay_View_Helper::format_number($laterpay_global_default_price, 2); ?>"><?php _e('Global Default<span></span> Price', 'laterpay'); ?></a>
        </li>
    </ul>
            ...
            <ul>
                <?php foreach($laterpay_category_prices as $c): ?>
                    <li data-category="<?php echo $c['category_id']; ?>"<?php if ($c['category_id'] == $laterpay_post_default_category):?> class="selected-category"<?php endif;?>>
                        <a href="#" data-price="<?php echo LaterPay_View_Helper::format_number($c['category_price'], 2); ?>">
                            <span><?php echo LaterPay_View_Helper::format_number($c['category_price'], 2); ?> <?php echo $laterpay_currency; ?></span><?php echo $c['category_name']; ?>
                        </a>
                    </li>
                <?php endforeach; ?>
            </ul>
