...
if( ! class_exists( 'Plugin_Usage_Tracker') ) {
    ...
    class Plugin_Usage_Tracker {
         ...
         * @param array  $_options              Plugin options to track
         * @param bool   $_require_optin        Whether user opt-in is required (always required on WordPress.org)
         * @param bool   $_include_goodbye_form Whether to include a form when the user deactivates
         * @param bool   $_marketing            Marketing method:
        ...
        public function get_data() {
            ...
            $body = array(
                ...
                'message'        => '', // Use this to pass error messages back if necessary.
            );
            ...
            $body['server'] = isset( $_SERVER['SERVER_SOFTWARE'] ) ? wp_unslash( $_SERVER['SERVER_SOFTWARE'] ) : ''; // phpcs:ignore
        ...
        public function optin_notice() {
            ...
            if( isset( $_GET['plugin'] ) && isset( $_GET['plugin_action'] ) ) { // phpcs:ignore WordPress.VIP.SuperGlobalInputUsage.AccessDetected
                $nonce = filter_input( INPUT_GET, '_wpnonce', FILTER_SANITIZE_STRING );

                // Verify nonce.
                if ( ! wp_verify_nonce( $nonce, 'lp_wisdom_optin' ) ) {
                    return;
                }

                $plugin = sanitize_text_field( $_GET['plugin'] ); // phpcs:ignore
                $action = sanitize_text_field( $_GET['plugin_action'] ); // phpcs:ignore
                if( $action === 'yes' ) {
            ...
            if ( $is_local ) {
                ...
                // Create nonce for optin verification.
                $nonce = wp_create_nonce( 'lp_wisdom_optin' );
        ...
        public function marketing_notice() {
            ...
            if( isset( $_GET['marketing_optin'] ) ) { // phpcs:ignore WordPress.VIP.SuperGlobalInputUsage.AccessDetected

                $nonce = filter_input( INPUT_GET, '_wpnonce', FILTER_SANITIZE_STRING );

                // Verify nonce.
                if ( ! wp_verify_nonce( $nonce, 'lp_wisdom_marketing_optin' ) ) {
                    return;
                }

                // Set marketing optin
                $this->set_can_collect_email( sanitize_text_field( $_GET['marketing_optin'] ), $this->plugin_name ); //phpcs:ignore WordPress.VIP.SuperGlobalInputUsage.AccessDetected
                // Do tracking
                $this->do_tracking( true );
            } elseif( isset( $_GET['marketing'] ) && $_GET['marketing'] === 'yes' ) { //phpcs:ignore WordPress.VIP.SuperGlobalInputUsage.AccessDetected
                // Display the notice requesting permission to collect email address
                // Retrieve current plugin information
                $plugin = $this->plugin_data();
                $plugin_name = $plugin['Name'];

                // Create nonce for marketing optin verification.
                $nonce = wp_create_nonce( 'lp_wisdom_marketing_optin' );
            ...
            <script>
                jQuery(document).ready(function($){
                    $("#put-goodbye-link-<?php echo esc_attr( $this->plugin_name ); ?>").on("click",function(){
                        ...
                        $("#put-goodbye-form-<?php echo esc_attr( $this->plugin_name ); ?>").empty().append( '<?php echo $html; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- already escaped above. ?>' + '<div class="put-goodbye-form-footer"><p><a id="put-submit-form" class="button primary" href="#"><?php esc_html_e( 'Submit and Deactivate', 'singularity' ); ?></a>&nbsp;<a class="secondary button" href="'+url+'"><?php esc_html_e( 'Just Deactivate', 'singularity' ); ?></a></p></div>');
                        $('#put-submit-form').on('click', function(e){
                            ...
                            var data = {
                                ...
                                'values': values,
                                'details': details,
                                'security': "<?php echo wp_create_nonce ( 'wisdom_goodbye_form' ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>",
                                'dataType': "json"
                            }
                            $.post(
                                ajaxurl,
        ...
        public function goodbye_form_callback() {
            ...
            if( isset( $_POST['values'] ) ) { //phpcs:ignore
                $values = wp_json_encode( wp_unslash( $_POST['values'] ) ); // phpcs:ignore
                update_option( 'wisdom_deactivation_reason_' . $this->plugin_name, $values );
            }
            if( isset( $_POST['details'] ) ) { //phpcs:ignore
                $details = sanitize_text_field( $_POST['details'] ); //phpcs:ignore
