...
<div id="lp_js_debugger" class="lp_debugger lp_is-hidden">
    <header id="lp_js_toggleDebuggerVisibility" class="lp_debugger-header">
        ...
        <div class="lp_debugger-header__text lp_right"><?php echo sprintf( __( '%s Memory Usage', 'laterpay' ), number_format( $laterpay_records['memory_peak'], 1 ) . ' MB' ); ?></div>
    ...
    <ul id="lp_js_debuggerTabs" class="lp_debugger-tabs lp_clearfix">
        <li class="lp_js_debuggerTabItem lp_is-selected lp_debugger-tabs__item">
            <a href="#" class="lp_debugger-tabs__link"><?php echo sprintf( __( 'Messages<span class="lp_badge">%s</span>', 'laterpay' ), count( $laterpay_records['records'] ) ); ?></a>
        </li>
        <?php
