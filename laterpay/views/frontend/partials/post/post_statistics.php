    ...
    <div class="lp_post-statistics__totals">
        <ul class="lp_post-statistics__list">
            <li class="lp_post-statistics__item">
                <div class="lp_kpi">
                    <big class="lp_kpi__value"><?php if ( isset( $laterpay['statistic']['total'][$currency] ) ) { $aux = $laterpay['statistic']['total'][$currency]['sum']; } else { $aux = 0; }; echo LaterPay_Helper_View::format_number( (float) $aux ); ?><small class="lp_kpi__text"><?php echo $laterpay['currency']; ?></small></big>
                    <small class="lp_kpi__text"><?php _e( 'Total Committed Revenue', 'laterpay' ); ?></small>
                </div>
            </li>
            <li class="lp_post-statistics__item">
                <div class="lp_kpi">
                    <big class="lp_kpi__value"><?php if ( isset( $laterpay['statistic']['total'][$currency] ) ) { $aux = $laterpay['statistic']['total'][$currency]['quantity']; } else { $aux = 0; }; echo LaterPay_Helper_View::format_number( (float) $aux, false ); ?></big>
                    <small class="lp_kpi__text"><?php _e( 'Total Sales', 'laterpay' ); ?></small>
                </div>
            </li>
        </ul>
    </div>
